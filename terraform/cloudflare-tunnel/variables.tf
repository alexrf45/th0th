variable "cloudflare_api_token" {
  description = <<-EOT
    ...
    Tunnel: Edit). Add "Zone · DNS · Edit" for home-0ps.com when the public
    hostname/DNS record is added in G2. Prefer an account-owned token.
    Separate from the cert-manager DNS-01 token (cf_token_home-0ps.com).
  EOT
  type        = string
  sensitive   = true
}
...
variable "cloudflare_account_id" {
  description = "Cloudflare account ID that owns the home-0ps.com zone."
  type        = string
}
