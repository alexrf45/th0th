...
| O-12 | PR CI lacks PromQL alert-template validation | ❌ Open | `.github/workflows/lint.yaml` + `_lib/observability/kube-prometheus-stack/prometheusrule-*.yaml` | Sprig functions (`mul`, `div`, `add`, `default`, etc.) parse-pass YAML lint + CRD schema but fail at the Prometheus Operator's mutating webhook — alert templates use Go `text/template` + Prometheus extensions only. Example: PVCNearFull's `(mul $value 100)` shipped via O-7 (PR #50) and only surfaced when Flux re-applied during the 2026-05-29 kromgo reconcile (`11f008e`); fix in follow-on. The pattern stays open. **Next action:** add `promtool check rules` over rendered PrometheusRule manifests in the lint workflow — catches the same class of bug pre-merge. |
