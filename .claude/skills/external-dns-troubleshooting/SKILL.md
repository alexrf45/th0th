...
Diagnose failures in ExternalDNS — the controller that synchronises Kubernetes Service/Ingress annotations to DNS providers (Route53, Azure DNS, CloudFlare, Google Cloud DNS, etc.).
...
| Force sync (recommend to user) | `kubectl rollout restart deploy/external-dns -n external-dns` |
...
| Problem | Symptom in Logs | Diagnostic |
|---------|-----------------|-----------|
| Domain filter too strict | "0 zones match" or skipped records | Check `--domain-filter` scope; recommend widening or adding `--exclude-domains` |
| Zone doesn't exist at provider | "zone not found" | Verify zone exists at the provider |
| Zone ID filter wrong | Records in wrong zone | Check `--zone-id-filter` argument against provider zone IDs |
| Sub-domain not covered | Parent zone exists but child not matched | Check if sub-domain is included in domain filter |
...
|---------|---------|-----------|
| Planned but not applied | "Creating" in logs, record missing at provider | Check if `--dry-run` flag is set in deployment args |
| Rate limited | 429 errors or "rate limit" messages | Check `--interval` value and record count |
...
| Cause | Check | Diagnostic |
|-------|-------|-----------|
| `--namespace` flag restricts scope | Args show `--namespace=X` | Check if namespace filter is intentional or too restrictive |
| RBAC too restrictive | `auth can-i` returns "no" | Check ClusterRole permissions for services/ingresses/nodes |
| No annotated resources exist | Services/Ingresses lack annotations | Check if `external-dns.alpha.kubernetes.io/hostname` annotation is present |
| Interval too long | Args show `--interval=60m` | Check `--interval` value; recommend reducing for faster feedback |
...
|---------|-------|-----------|
| Record exists but ExternalDNS ignores it | TXT record has different owner ID | Compare `--txt-owner-id` with TXT record owner; check for mismatch |
| Multiple clusters fighting over records | Same `--txt-owner-id` on different clusters | Check if each cluster has a unique owner ID |
| Records deleted unexpectedly | `--policy=sync` removes unmanaged records | Check if `--policy=sync` is set; recommend `upsert-only` if unintended |
| Orphaned records after cluster deletion | No cleanup ran | Check provider console for orphaned DNS and TXT ownership records |
| TXT prefix collision | Default prefix `a-` or `cname-` collides | Check `--txt-prefix` for uniqueness |
